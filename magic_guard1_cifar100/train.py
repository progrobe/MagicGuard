...
from models import densenet, ResNet18, vgg19_bn,mobilenet
...
parser.add_argument('--lradj', default=7, type=int, help='multiple the lr by 0.1 every n epochs')
...
# net = torch.load('checkpoint/cifar100/pre_trained/pre_trained_mobilenetv2.pkl')
# net = torch.load('checkpoint/cifar100/pre_trained/pre_trained_mobilenet.pkl')
net = mobilenet()
...
for epoch in range(start_epoch, start_epoch + args.max_epochs):
    ...
    if acc > best_acc:
        print('saving...')
        torch.save(net, 'checkpoint/cifar100/pre_trained/pre_trained_mobilenet.pkl')
        best_acc = acc
